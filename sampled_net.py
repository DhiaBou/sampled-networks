...
class SampledNet(Model):
    ...
    def fit(self, X_train, y_train, model: NeuralNet, radius=0, validation_split = 0.2, layer2="classic", alpha=1, num_intervals=10):
        """
        ...
        if layer2 =="ridge" and radius == -1 and alpha == -1:
            ...
            alpha, radius, self.weights, self.biases = choose_best_radius_alpha(X_train, y_train, X_val, y_val, model.weights, model.biases, num_intervals=num_intervals)
        
        elif layer2 =="ridge" and radius != -1 and alpha == -1:
            X_train, X_val, y_train, y_val = train_test_split(X_train, y_train, test_size=validation_split)
            alpha, self.weights, self.biases = choose_best_alpha(X_train, y_train, X_val, y_val, model.weights, model.biases, radius=radius)
        
        elif layer2 =="ridge" and radius == -1 and alpha != 1:
            raise ValueError("cannot determine best radius given alpha")
            
        else:
            weights_l1, biases_l1 = compute_weights_biases_layer1(X_train, y_train, model.weights, model.biases, radius)
            weights_l2, biases_l2 = self._compute_weights_biases_layer2(X_train, y_train, model.weights, model.biases, weights_l1, biases_l1, layer2=layer2, alpha=alpha)
    ...
    def _compute_weights_biases_layer2(self, X, y, weights, biases, weights_l1, biases_l1, alpha=1, layer2 = "classic"):
        if layer2 == "classic":
            return compute_weights_biases_layer2_classic(
                X, y, weights, biases, weights_l1, biases_l1
            )
        elif layer2 == "lstsq":
            return compute_weights_biases_layer2_lstsq(
                X, y, weights_l1, biases_l1
            )
        elif layer2 == "ridge":
